    ...
    switch (d->kind)
    {
        case A_varDec:
        {
            ...
            if (init != NULL) {
                ...
                init_type = transExp(venv, tenv, init, OUTSIDE_LOOP);

                // printf("init_type retrieved. init_type: %d \n", init_type);
                // printf("init_type retrieved. init_type.ty: %d \n", init_type.ty);
            ...
            else 
            {
                ...
                // it is not allowed to have no initializer
                assert(0);
            }

            // the type specifier on a variable is optional so take care
            // here to not dereference a null pointer
            S_symbol type_symbol = typ;
            if (type_symbol == NULL)
            {
                // if the variable is declared without type, use the init type which
                // is mandatory and always present
                S_enter(venv, var, init_type.ty);
            ...
            else
            {
                ...
                if (var_type != init_type.ty) {
                    EM_error(d->pos, "Array of type \"%s\" is initialized with a different array type!\n", S_name(type_symbol));
                    assert(0);
                }
